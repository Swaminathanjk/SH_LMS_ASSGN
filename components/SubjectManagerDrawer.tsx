...
const SubjectManagerDrawer: React.FC<SubjectManagerDrawerProps> = ({
  student,
  studentSubjects,
  onSave,
  onClose,
  ...
  useEffect(() => {
    const initialData = studentSubjects
      ? JSON.parse(JSON.stringify(studentSubjects))
      : [];
  ...
  const handleApplyAiSubjects = (aiSubjects: SubjectData[]) => {
    setSubjects((prevSubjects) => {
      const existingSubjectNames = new Set(
        prevSubjects.map((s) => s.subject.toLowerCase().trim())
      );

      // Filter out AI subjects that already exist
      const newSubjects = aiSubjects.filter(
        (aiSubject) =>
          !existingSubjectNames.has(aiSubject.subject.toLowerCase().trim())
      );
  ...
  const handleCancel = () => {
    setSubjects(
      studentSubjects ? JSON.parse(JSON.stringify(studentSubjects)) : []
    );
    setIsEditMode(false);
    setErrors({});
  };
  ...
  const handleInputChange = (
    sIdx: number,
    cIdx: number | null,
    field: keyof SubjectData | keyof Chapter,
    value: string
  ) => {
  ...
  const deleteChapter = (sIdx: number, cIdx: number) => {
    const newSubjects = JSON.parse(JSON.stringify(subjects));
    newSubjects[sIdx].chapters = newSubjects[sIdx].chapters.filter(
      (_, i) => i !== cIdx
    );
    setSubjects(newSubjects);
  };
        ...
        <div key={subject.subject}>
          ...
          <div className="mt-2 pl-2 space-y-2 border-l-2 border-gray-200 dark:border-gray-700">
            {subject.chapters.map((chapter) => (
              <div
                key={`${chapter.no}-${chapter.name}`}
                className="flex items-start gap-3 text-gray-700 dark:text-gray-300 ml-2"
                title={chapter.name}
              >
                <span className="w-8 pt-0.5 text-right font-mono text-sm text-gray-400 dark:text-gray-500">
                  {chapter.no}.
                </span>
  ...
  const editContent = () => (
    <div className="space-y-4">
      {subjects.map((subject, sIdx) => (
        <div
          key={sIdx}
          className="bg-light-bg/50 dark:bg-dark-bg/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700"
        >
          <div>
            <div className="flex items-center">
              <input
                ...
                onChange={(e) =>
                  handleInputChange(sIdx, null, "subject", e.target.value)
                }
                placeholder="Subject Name"
                className={`text-lg font-semibold w-full bg-transparent p-1 focus:outline-none border-b-2 ${
                  errors[sIdx]?.subject
                    ? "border-red-500"
                    : "border-gray-300 dark:border-gray-600 focus:border-brand-blue dark:focus:border-brand-blue"
                }`}
              />
              <button
          ...
          <div className="mt-3 pl-4 space-y-3">
              ...
              <div key={cIdx}>
                <div className="flex items-center space-x-2">
                  <input
                    value={chapter.no}
                    onChange={(e) =>
                      handleInputChange(sIdx, cIdx, "no", e.target.value)
                    }
                    placeholder="No."
                    className={`w-16 p-2 rounded-md bg-white dark:bg-gray-800 border ${
                      errors[sIdx]?.chapters?.[cIdx]?.no
                        ? "border-red-500"
                  ...
                  <input
                    value={chapter.name}
                    onChange={(e) =>
                      handleInputChange(sIdx, cIdx, "name", e.target.value)
                    }
                    placeholder="Chapter Name"
                    className={`w-full p-2 rounded-md bg-white dark:bg-gray-800 border ${
                      errors[sIdx]?.chapters?.[cIdx]?.name
                        ? "border-red-500"
                        : "border-gray-300 dark:border-gray-600"
                    }`}
                  />
                  <button
                    onClick={() => deleteChapter(sIdx, cIdx)}
            ...
            <button
              ...
              className="text-brand-blue font-semibold text-sm hover:underline mt-2"
            >
              + Add Chapter
            </button>
          </div>
      ...
      <button
        onClick={addSubject}
        className="w-full text-center mt-2 py-2 bg-gray-200/50 dark:bg-gray-700/50 rounded-lg font-semibold hover:bg-gray-200 dark:hover:bg-gray-700 border border-gray-300/50 dark:border-gray-600/50 transition-colors"
      >
        + Add Subject
      </button>
    </div>
  ...
  const noSubjectsContent = () => (
    <div className="text-center py-16 px-4 bg-gray-50 dark:bg-dark-bg/30 rounded-lg">
      <h4 className="font-semibold text-lg text-gray-700 dark:text-gray-300">
        No subjects set for {student.name}.
      </h4>
      <p className="text-gray-500 dark:text-gray-400 mt-1">
        Get started by asking AI or entering them manually.
      </p>
      <div className="mt-6 flex justify-center gap-4">
        <button
          onClick={handleEnterManually}
          className="flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold bg-gray-100 text-gray-700 border border-gray-200 hover:bg-gray-200 transition-colors dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600 dark:hover:bg-gray-600"
        >
  ...
  const renderContent = () => {
    if (isArchived || (!isEditMode && subjects.length > 0))
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-2xl h-full bg-light-card dark:bg-dark-card shadow-2xl flex flex-col relative overflow-hidden"
      ...
      >
          ...
          <div className="flex justify-between items-start">
            ...
            {!isArchived && !isEditMode && subjects.length > 0 && (
              <button
                ...
                className="flex items-center py-2 px-4 rounded-lg font-semibold bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-sm hover:bg-gray-50 dark:hover:bg-gray-600"
              >
        ...
        <main className="flex-grow overflow-y-auto p-6">{renderContent()}</main>
            ...
            {isEditMode && !isArchived ? (
              <div className="flex justify-end space-x-3">
                <button
                  ...
                  className="py-2 px-5 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-500 font-semibold"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSave}
                  className="h-10 px-4 rounded-md bg-brand-blue text-white hover:bg-blue-600 text-sm font-semibold"
                >
                  Save Changes
                </button>
              </div>
              ...
              <div className="flex justify-end">
                <button
                  onClick={onClose}
                  className="py-2 px-5 rounded-lg bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 font-semibold"
                >
                  Close
                </button>
              </div>
